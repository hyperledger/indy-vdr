...
impl PoolService {
    ...
    pub fn create(&self, name: &str, config: Option<PoolConfig>) -> LedgerResult<()> {
        ...
        if path.as_path().exists() {
            return Err(err_msg(
                LedgerErrorKind::PoolConfigAlreadyExists,
                format!(
        ...
        fs::create_dir_all(path.as_path()).to_result(
            LedgerErrorKind::IOError,
            "Can't create pool config directory",
        )?;
...

        {
            ...
            let mut gt_fin = fs::File::open(&pool_config.genesis_txn).to_result(
                LedgerErrorKind::IOError,
                format!("Can't open genesis txn file {:?}", &pool_config.genesis_txn),
            )?;

            let mut gt_fout = fs::File::create(path.as_path()).to_result(
                LedgerErrorKind::IOError,
                format!("Can't create genesis txn file {:?}", path.as_path()),
            )?;

            io::copy(&mut gt_fin, &mut gt_fout).to_result(
                LedgerErrorKind::IOError,
                format!(
        ...
        let mut f: fs::File = fs::File::create(path.as_path())
            .to_result(LedgerErrorKind::IOError, "Can't create pool config file")?;

        f.write_all({
            serde_json::to_string(&pool_config)
                .to_result(LedgerErrorKind::InvalidState, "Can't serialize pool config")?
                .as_bytes()
        })
        .to_result(LedgerErrorKind::IOError, "Can't write to pool config file")?;
    ...
    pub fn delete(&self, name: &str) -> LedgerResult<()> {
        ...
        fs::remove_dir_all(path).to_result(
            LedgerErrorKind::IOError,
            "Can't delete pool config directory",
        )
    }

    pub fn open(&self, name: &str, config: Option<PoolOpenConfig>) -> LedgerResult<PoolHandle> {
        for ref pool in self.open_pools.try_borrow()?.values() {
            if name.eq(pool.pool.get_name()) {
                //TODO change error
                return Err(err_msg(
                    LedgerErrorKind::InvalidPoolHandle,
    ...
    pub fn add_open_pool(&self, pool_id: PoolHandle) -> LedgerResult<PoolHandle> {
        let pool = self
            .pending_pools
            .try_borrow_mut()?
            .remove(&pool_id)
            .ok_or_else(|| {
                err_msg(
                    LedgerErrorKind::InvalidPoolHandle,
                    format!("No pool with requested handle {:?}", pool_id),
                )
            })?;
    ...
    pub fn send_tx(&self, handle: PoolHandle, msg: &str) -> LedgerResult<CommandHandle> {
    ...
    pub fn register_sp_parser(
        ...
        if events::REQUESTS_FOR_STATE_PROOFS.contains(&txn_type) {
            return Err(err_msg(
                LedgerErrorKind::InvalidStructure,
                format!(
                    "Try to override StateProof parser for default TXN_TYPE {}",
                    txn_type
                ),
            ));
        }

        REGISTERED_SP_PARSERS
            .lock()
            .map(|mut map| {
    ...
    pub fn close(&self, handle: PoolHandle) -> LedgerResult<CommandHandle> {
        let cmd_id: CommandHandle = next_command_handle();
    ...
    pub fn refresh(&self, handle: PoolHandle) -> LedgerResult<i32> {
    ...
    pub fn list(&self) -> LedgerResult<Vec<serde_json::Value>> {
...
pub fn parse_response_metadata(response: &str) -> LedgerResult<ResponseMetadata> {
    ...
    let message: Message<serde_json::Value> = serde_json::from_str(response).to_result(
...
fn _handle_response_message_type<T>(message: Message<T>) -> LedgerResult<Reply<T>>
where
