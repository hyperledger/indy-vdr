...
if [ "$TARGET" = "apple-darwin" ]; then
	...
	for target in $MACOS_UNIVERSAL_TARGETS; do
		...
		$RUSTUP run $BUILD_TOOLCHAIN cargo build --manifest-path libindy_vdr/Cargo.toml --release --features $FEATURES --target $target
	...
	CMD="$RUSTUP run $BUILD_TOOLCHAIN cargo build --manifest-path libindy_vdr/Cargo.toml --release --features $FEATURES"
	if [ -n "$TARGET" ]; then
		$CMD --target "$TARGET"
	else
		$CMD
