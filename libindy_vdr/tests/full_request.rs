...
fn test_pool_send_full_request_works(pool: TestPool, trustee: Identity) {
    let replies = sign_and_send_full_request(&pool,
                                             &trustee,
                                             None,
                                             None).unwrap();
...
fn test_pool_send_full_request_works_for_unknown_node(pool: TestPool, trustee: Identity) {
    let _err = sign_and_send_full_request(&pool,
                                          &trustee,
                                          Some(vec![String::from("UNKNOWN")]),
                                          None).unwrap_err();
}

fn sign_and_send_full_request(pool: &TestPool,
                              trustee: &Identity,
                              node_aliases: Option<Vec<String>>,
                              timeout: Option<i64>) -> VdrResult<NodeReplies<String>> {
    let mut request =
